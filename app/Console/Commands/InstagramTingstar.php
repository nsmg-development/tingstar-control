...
class InstagramTingstar extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'scrap:tingstar:instagram:keyword';
     ...
     * @var string
     */
    protected $description = '인스타그램 크롤링(키워드) - 팅스타';
                        ...
                        try {
                            ...
                            if (!$article) {
                                ...
                                if ($node->getUserThumbnail()) {
                                    $thumbnail = $this->azureService->AzureUploadImage($node->getUserThumbnail(), date('Y') . '/tingstar/images');
                                ...
                                if ($node->getImageUrl()) {
                                    foreach ($node->getImageUrl() as $value) {
                                        if ($node->getId() === $value['carousel_parent_id']) {
                                            ...
                                            $thumbnail = $this->azureService->AzureUploadImage($img, date('Y') . '/tingstar/images');
                                            $size = getimagesize($this->storageBaseUrl . $thumbnail);
                                            $width = $size[0];
                                            $height = $size[1];
                                            $mime = $size['mime'];
